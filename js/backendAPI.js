...
class BackendAPI {
  ...
  getLocalGraph(options = {}) {
    ...
    const transformedNodes = [
      ...
      { id: 'light_reactions', label: 'Light Reactions', size: 35, type: 'process' }
    ];

    const transformedLinks = [
      { source: 'photosynthesis', target: 'chloroplast', weight: 8, label: 'occurs_in' },
      { source: 'photosynthesis', target: 'light_reactions', weight: 7, label: 'includes' }
    ];
    ...
    return {
      ...
      source: 'local_sample',
      message: 'Local sample data loaded'
    };
  }

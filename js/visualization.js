...
class GraphVisualization {
    ...
    showLinkTooltip(event, d) {
        const sourceLabel = d.source.label || d.source.id || 'Unknown';
        const targetLabel = d.target.label || d.target.id || 'Unknown';
        ...
        tooltip.innerHTML = `
            ...
            Weight: ${d.weight}<br>
            Strength: ${d.weight > 5 ? 'Strong' : d.weight > 2 ? 'Medium' : 'Weak'}
        `;
    ...
    renderSimpleGraph() {
        if (!this.svg || !this.visibleNodes || this.visibleNodes.length === 0) {
            console.log('Cannot render:', !this.svg, !this.visibleNodes, this.visibleNodes?.length);
            return;
        }

        console.log('Starting simple graph rendering');

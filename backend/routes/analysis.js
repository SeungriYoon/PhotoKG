...
// [NEW] PEO Coverage Analysis
router.post('/peo', async (req, res) => {
  try {
    ...
    if (!papers_data || !Array.isArray(papers_data)) {
      return res.status(400).json({
        success: false,
        error: 'Invalid papers_data format. Expected an array of papers.'
      });
    }
    ...
    res.json({
      success: true,
      data: result,
      message: 'PEO coverage analysis completed successfully',
      analysis_type: 'peo_coverage',
